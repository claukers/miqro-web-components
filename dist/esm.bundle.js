...
let basePath = null;
const BASE_PATH = ()=>basePath === null ? normalizePath(document.documentElement.getAttribute("data-router-base-path") ? document.documentElement.getAttribute("data-router-base-path") : "") : basePath;
function isPathLocation(p) {
    if (p === undefined) {
        return false;
    }
...
function isRouteActive(route, activeRoute, useDefault = true) {
...
function getActiveRoute(routes, currentActive) {
    let activeRoute;
...
function historyPushPath(path) {
    window.history.pushState(null, "", BASE_PATH() + path);
    windowDispatchEvent(new PopStateEvent("popstate"));
}
const renderTag = (tagName)=>typeof tagName === "function" ? tagName() : `<${tagName}/>`;
function PathRouter() {
    ...
    const [activeRoute, setActiveRoute, getActive] = this.useState(currentActive ? currentActive : {
    ...
    const activeElement = activeRoute ? activeRoute.element : undefined;
    return activeElement ? renderTag(activeElement) : "";
}
function flipActiveClass(element, path, activeClass) {
...
function RouteLink() {
    ...
    const activeClass = this.useAttribute("data-active-class", "active", false);
...
class Store extends EventTarget {
    constructor(reducers, state, options){
        super();
    ...
    subscribe(selector, listener) {
        ...
        mapSet.call(this.listenerMap, listener, {
            lastResult,
            selector
        });
        return lastResult;
    ...
    dispatch(action) {
        ...
        this.state = typeof this.reducers === "function" ? this.reducers(action, oldState) : this.reducers[action.type] ? this.reducers[action.type](action, oldState) : oldState;
        return this.queueTrigger();
    }
    queueTrigger() {
        clearTimeout(this.triggerListenerTimeout);
        this.triggerListenerTimeout = setTimeout(()=>triggerListeners(this, this.listenerMap), this.options.dispatchTimeout);
    }
}
function triggerListeners(store, listenerMap) {
    const state = store.getState();
    const listeners = mapKeys.call(listenerMap);
    for (const listener of listeners){
        const listenerInfo = mapGet.call(listenerMap, listener);
        if (listenerInfo) {
            try {
                ...
                store.dispatchEvent(new ErrorEvent(String(e) + " " + String(e.stack)));
            }
        }
    }
}
export { define as define };
export { setCache as setCache };
export { PathRouter as PathRouter, RouteLink as RouteLink, historyPushPath as historyPushPath };
